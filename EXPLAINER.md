...
### Catch 4: Missing row lock and broken backoff in payout processor

**The bug:** AI wrote the `process_payout` Celery task without `select_for_update()` and placed the `attempts` / `last_attempted_at` update inside the `if pending` block.
...
**What I caught:** Two problems:

1. No `select_for_update()` — if `retry_stuck_payouts` fires at the same time as a delayed `process_payout`, both could read the same payout, both could try to transition it, and both could insert ledger entries. That's a double credit or double debit on the merchant's balance.

2. `attempts` and `last_attempted_at` only update when status is `pending`. On retries (status is already `processing`), this block is skipped entirely. `last_attempted_at` never updates, so the retry checker keeps picking it up every 30 seconds instead of respecting exponential backoff. Backoff is completely broken.
...
One outer `transaction.atomic()` with `select_for_update()` — the lock, status check, transition, and ledger entry all happen atomically. `attempts` and `last_attempted_at` update on every attempt, not just the first, so exponential backoff actually works.